...
const HowItWorksSection = () => {
  ...
  const StepCard = ({ step, isLast }: { step: any, isLast: boolean }) => (
    <div className="group relative p-12 bg-white rounded-none border border-border/40 hover:shadow-2xl hover:shadow-black/5 transition-all duration-500 hover:-translate-y-1">
      <div className="absolute top-6 right-6 text-6xl font-black text-foreground/5 group-hover:text-primary/10 transition-colors">
        {step.step}
      </div>
      
      <div className="w-12 h-12 rounded-none bg-foreground text-white flex items-center justify-center mb-10 group-hover:bg-primary transition-colors duration-500">
        <step.icon className="w-6 h-6" />
      </div>
      
      <div className="space-y-4 relative z-10">
        <h3 className="text-xl font-black text-foreground tracking-tight uppercase leading-tight">{step.title}</h3>
        <p className="text-muted-foreground font-bold text-[13px] leading-relaxed max-w-[200px]">{step.description}</p>
      </div>
    ...
    <section className="py-32 bg-[#fafafa] overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center max-w-4xl mx-auto mb-20">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-none bg-foreground text-white text-[10px] font-black uppercase tracking-widest mb-8">
        ...
        <Tabs defaultValue="buyer" className="w-full">
          <div className="flex justify-center mb-16">
            <TabsList className="h-16 p-0 bg-transparent rounded-none flex gap-0 border border-border/40 overflow-hidden">
              <TabsTrigger
                value="buyer"
                className="w-48 lg:w-64 h-full rounded-none text-[11px] font-black uppercase tracking-widest data-[state=active]:bg-foreground data-[state=active]:text-white transition-all duration-300 border-r border-border/40"
              >
                Buying Experience
              </TabsTrigger>
              <TabsTrigger
                value="vendor"
                className="w-48 lg:w-64 h-full rounded-none text-[11px] font-black uppercase tracking-widest data-[state=active]:bg-foreground data-[state=active]:text-white transition-all duration-300"
              >
                Selling Journey
              </TabsTrigger>
            </TabsList>
