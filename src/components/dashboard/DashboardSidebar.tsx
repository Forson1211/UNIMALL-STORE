      ...
      <Sidebar className={`${collapsed ? "w-16" : "w-64"} bg-[#0B132B] text-slate-300 border-r border-slate-800/80 transition-all duration-300`} collapsible="icon" data-state={state}>
        ...
        <SidebarFooter className="border-t border-slate-800/80 p-3 bg-[#0B132B]">
          <Button
            ...
            className="w-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800/60 mb-2 h-8 rounded-lg cursor-pointer"
          >
          ...
          <SidebarMenu className="space-y-1">
            {role === 'admin' && (
              <SidebarMenuItem>
                <SidebarMenuButton 
                  asChild 
                  tooltip={collapsed ? (type === 'admin' ? "Seller Dashboard" : "Admin Hub") : undefined} 
                  onClick={handleMobileClick} 
                  className="text-orange-300 hover:text-white hover:bg-orange-500/20 rounded-xl font-bold transition-colors cursor-pointer"
                >
                  <Link to={type === 'admin' ? "/vendor" : "/admin"} className="flex items-center gap-3">
                    {type === 'admin' ? <Store className="w-4 h-4 shrink-0 text-[#FF5500]" /> : <ShieldCheck className="w-4 h-4 shrink-0 text-[#FF5500]" />}
                    {!collapsed && <span className="text-xs">{type === 'admin' ? "Seller Dashboard" : "Admin Hub"}</span>}
                  </Link>
                </SidebarMenuButton>
              </SidebarMenuItem>
            )}
            <SidebarMenuItem>
              <SidebarMenuButton asChild tooltip={collapsed ? "Back to Store" : undefined} onClick={handleMobileClick} className="text-slate-400 hover:text-white hover:bg-slate-800/60 rounded-xl cursor-pointer">
            ...
            <SidebarMenuItem>
              <SidebarMenuButton asChild tooltip={collapsed ? "Logout" : undefined} onClick={handleMobileClick} className="text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 rounded-xl cursor-pointer">
