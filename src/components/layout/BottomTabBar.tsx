...
const BottomTabBar = () => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-card border-t border-gray-200 dark:border-border shadow-[0_-4px_16px_rgba(0,0,0,0.06)] h-[58px] pt-2 pb-1.5 px-1 flex items-center justify-around">
      {/* 1. Home */}
      <Link
        to="/"
        className={`flex flex-col items-center justify-center gap-1 flex-1 h-full text-[10.5px] font-medium transition-colors ${
      ...
      <button
        ...
        className={`flex flex-col items-center justify-center gap-1 flex-1 h-full text-[10.5px] font-medium transition-colors focus:outline-none ${
          isMenuOpen ? "text-[#FF5500]" : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"
        }`}
      >
      ...
      <button
        onClick={openCart}
        className={`flex flex-col items-center justify-center gap-1 flex-1 h-full text-[10.5px] font-medium transition-colors focus:outline-none relative ${
          isCartOpen ? "text-[#FF5500]" : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"
        }`}
      >
        <MCBCartIcon
          count={totalItems}
          iconClassName="w-[18px] h-[18px] text-current"
          badgeClassName="absolute -top-1 -right-1.5 bg-[#FF5500] text-white text-[8.5px] font-bold min-w-[15px] h-[15px] px-0.5 rounded-full flex items-center justify-center shadow-xs leading-none"
        />
        <span>Cart</span>
      </button>
